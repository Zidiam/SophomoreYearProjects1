...
public class ExpressionTree extends LinkedBinaryTree<ExpressionTreeOp>
{
	...
	/**
	 * Calls the printInfix method that takes a node and String parameter
	 * 
	 * @return a string infix representation of the tree
	 */
	public String printInfix() {
		String fin = printInfix(this.root, "");
		return fin.substring(1, fin.length()-1);
		
	}
	
	/**
	 * Uses a inOrder sort to make an infix representation of a tree
	 * This is a recursive method
	 * 
	 * @return a string infix representation of the tree
	 */
	
	public String printInfix(BinaryTreeNode node, String result) {
	if(node != null) {
		//If the node is a operator then start the parentheses for the result
		if(node.getElement().toString().equals("+") || node.getElement().toString().equals("-") || node.getElement().toString().equals("/") || node.getElement().toString().equals("*")) {
			result += "(";
		}
		result = printInfix(node.getLeft(), result);
		result = result + (node.getElement());
		result = printInfix(node.getRight(), result);
		//If the node is a operator then end result with a close parentheses this step happens after getting the left and right elements
		if(node.getElement().toString().equals("+") || node.getElement().toString().equals("-") || node.getElement().toString().equals("/") || node.getElement().toString().equals("*")) {
			result += ")";
		}
	}
